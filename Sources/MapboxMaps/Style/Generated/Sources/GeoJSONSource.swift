...
#endif
@_spi(Experimental) extension GeoJSONSource {
...
#endif
    /// Size of the tile buffer on each side. A value of 0 produces no buffer. A value of 512 produces a buffer as wide as the tile itself. Larger values produce fewer rendering artifacts near tile edges and slower performance.
    public func buffer(_ newValue: Double) -> Self {
        with(self, setter(\.buffer, newValue))
    }    
...
#endif
    /// Douglas-Peucker simplification tolerance (higher means simpler geometries and faster performance).
    public func tolerance(_ newValue: Double) -> Self {
        with(self, setter(\.tolerance, newValue))
    }    
    ...
    ///  * `point_count` Number of original points grouped into this cluster
    ///  * `point_count_abbreviated` An abbreviated point count
    public func cluster(_ newValue: Bool) -> Self {
        with(self, setter(\.cluster, newValue))
    }    
...
#endif
    /// Radius of each cluster if clustering is enabled. A value of 512 indicates a radius equal to the width of a tile.
    public func clusterRadius(_ newValue: Double) -> Self {
        with(self, setter(\.clusterRadius, newValue))
    }    
...
#endif
    /// Max zoom on which to cluster points if clustering is enabled. Defaults to one zoom less than maxzoom (so that last zoom features are not clustered). Clusters are re-evaluated at integer zoom levels so setting clusterMaxZoom to 14 means the clusters will be displayed until z15.
    public func clusterMaxZoom(_ newValue: Double) -> Self {
        with(self, setter(\.clusterMaxZoom, newValue))
    }    
    ...
    /// For more advanced use cases, in place of `operator`, you can use a custom reduce expression that references a special `["accumulated"]` value, e.g.:
    /// `{"sum": [["+", ["accumulated"], ["get", "sum"]], ["get", "scalerank"]]}`
    public func clusterProperties(_ newValue: [String: Expression]) -> Self {
        with(self, setter(\.clusterProperties, newValue))
    }    
...
#endif
    /// Whether to calculate line distance metrics. This is required for line layers that specify `line-gradient` values.
    public func lineMetrics(_ newValue: Bool) -> Self {
        with(self, setter(\.lineMetrics, newValue))
    }    
...
#endif
    /// Whether to generate ids for the geojson features. When enabled, the `feature.id` property will be auto assigned based on its index in the `features` array, over-writing any previous values.
    public func generateId(_ newValue: Bool) -> Self {
        with(self, setter(\.generateId, newValue))
    }    
