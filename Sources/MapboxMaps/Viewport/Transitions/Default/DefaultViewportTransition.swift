...
extension DefaultViewportTransition: ViewportTransition {
    public func run(from fromState: ViewportState?,
        ...
        resultCancelable.add(toState.observeDataSource { [self, options] cameraOptions in
            resultCancelable.add(self.animate(to: cameraOptions, maxDuration: options.maxDuration) { finished in
