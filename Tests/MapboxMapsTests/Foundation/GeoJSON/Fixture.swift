...
class Fixture {
    class func stringFromFileNamed(name: String) -> String {
        guard let path = Bundle.mapboxTests.path(forResource: name, ofType: "json") ??
                Bundle.mapboxTests.path(forResource: name, ofType: "geojson") else {
    ...
    class func geojsonData(from name: String) throws -> Data? {
        guard let path = Bundle.mapboxTests.path(forResource: name, ofType: "geojson") else {
            XCTAssert(false, "Fixture \(name) not found.")
            return nil
        }
