...
///
/// Assign the MapStyle configuration to the ``StyleManager/mapStyle`` to load the new style,
/// or update style import configurations.
///
...
public struct MapStyle: Equatable {
    ...
    ///
    /// - Note: For the better performance with large local Style JSON please consider loading style from the file system via the ``MapStyle/init(uri:importConfigurations:)`` initializer.
    ///
    /// - Parameters:
