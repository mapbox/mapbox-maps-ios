...
public final class Style: StyleProtocol {
    ...
    /// Sets the style 3D  lights.
    ///
    /// - Parameter lights: List of ``Ligh3DProtocol`` used during rendering are specified in this array. Must contain exactly one ``DirectionalLight`` and one ``AmbientLight``.
    /// - throws: An error describing why the operation is unsuccessful.
    @_spi(Experimental) public func set3DLights(_ lights: [Light3DProtocol]) throws {
        let lights = try lights.map { try $0.allStyleProperties() }
        try handleExpected {
            _styleManager.setStyleLightsForLights(lights)
        }
    }

    /// Gets the list of ``Light3DInfo`` used during rendering.
    @_spi(Experimental) public func get3DLights() -> [Light3DInfo] {
        _styleManager.getStyleLights().compactMap { info in
    ...
    /// - throws: An error describing why the operation is unsuccessful.
    @_spi(Experimental) public func set3DLightProperty(id: String, property: String, value: Any) throws {
        try handleExpected {
            _styleManager.setStyleLightPropertyForId(id, property: property, value: value)
        }
    }
    ...
    /// - Parameter id: The unique identifier of the style 3D light in lights list.
    /// - Parameter property: The style 3D light property name.
    @_spi(Experimental) public func get3DLightProperty(id: String, property: String) -> Any {
        _styleManager.getStyleLightProperty(forId: id, property: property).value
    }
