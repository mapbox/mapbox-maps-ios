...
def write_object_as_JSON_to_file(object, output_file):
...
#-------------------------------------------------------------------------------------------
def devicePool(project_arn, pool_type = None):
    """
    ...
    if type is not None:
        ...
        filtered_pools = [pool for pool in device_pools if pool["type"] == pool_type]
...
def wait_for_api_status(request_arn, api, completion_statuses, failure_statuses, delay, timeout = None, verbose = False):
    ...
    while True:
        ...
        elif response_status in failure_statuses:
            print("Response:")
            dumpJSON(response)
...
def wait_for_run(arn, timeout, verbose = False):
    ...
    #   COMPLETED
    #   STOPPING
    return wait_for_api_status(arn, "run", ['COMPLETED'], [], delay=300, verbose=verbose)
...
def list_artifacts(test_run_arn, output_dir = None, verbose = False):
    ...
    if output_dir is not None:
        arn = filesystemARN(test_run_arn)
...
def generate_dashboard_url(run_arn):
    ...
    url = "https://us-west-2.console.aws.amazon.com/devicefarm/home?region=us-east-1#/projects/"+m.group(1)+"/runs/"+m.group(2)
    return url
...
class Project:
    ...
    def create_upload(self, uploadType, inputFilename, outputFilename = None ):
        ...
        # Upload
        subprocess.check_output(['curl', '-T', inputFilename, response['upload']['url']], stderr=subprocess.STDOUT)
    ...
    def schedule_run(self, app_package_arn, test_package_arn, test_spec_arn, outputFilename = None ):
        ...
        try:
            response = subprocess.check_output(['aws', 'devicefarm', 'schedule-run', 
                '--project-arn', self.project_arn,
                '--app-arn', app_package_arn,
    ...
    def schedule_test(self, ipa_filename, xctest_filename, test_spec_filename):
        ...
        if test_package_type is not None:
            xctest_upload_response = self.create_upload(test_package_type, xctest_filename)
        ...
        if test_spec_filename is not None:
            test_spec_response = self.create_upload("XCTEST_UI_TEST_SPEC", test_spec_filename)
        ...
        # Schedule the test
        run_response = self.schedule_run(ipa_upload_arn, xctest_upload_arn, test_spec_arn)
...
if __name__ == "__main__":
    ...
    """
    parser = argparse.ArgumentParser(description='Script to configure, schedule and AWS Device Farm runs.', epilog=epilog)   
...
#TODO: clean up with multiple parsers
    ...
    parser.add_argument('--test-type', default='XCTEST', choices = ['BUILTIN_FUZZ', 'XCTEST', 'XCTEST_UI'], help= 'Indicates the type of tests being run; can be one of BUILTIN_FUZZ, XCTEST, XCTEST_UI')
    ...
    project = Project(args.name, args.arn, args.timeout, args.artifacts_dir, args.verbose, args.test_type, args.device_pool, args.output)
    
    if args.run_arn_file is not None:
        with open(args.run_arn_file, 'r') as input:
            run_response = json.load(input)
    ...
    if result == TESTS_ERRORED and args.fail_on_error == False:
